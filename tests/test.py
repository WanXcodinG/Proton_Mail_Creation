...
def test_automation(email,passwd):
    ...
    return otp
def test_read_file():
    file=File_handling(settings.filename,settings.headers)
...
def test_write(data):
    ...
    #file.one_line_write(settings.used_emails,data[0])
    return File_handling.one_line_read(settings.used_emails)
